# Form Asistensi

## Tugas Besar IF2150 - Rekayasa Perangkat Lunak

| Informasi | Keterangan |
| --- | --- |
| **Hari** | *Selasa* |
| **Tanggal** | *22/09/2026* |
| **Kelas** | *3* |
| **Nomor Kelompok** | *8*  |
| **Nama Kelompok** | *The Dragon Warrior*  |
| **Nama Perangkat Lunak** | *FoodLink*  |
| **Dokumen** | *K03_G08_CD.md*  |

### Anggota Kelompok

| NIM | Nama |
| --- | --- |
| *13525024* | *Excell Timothy Josua Tarigan* |
| *13525036* | *Dylan Frederico Ketaren* |
| *13525111* | *Edbert Fernando* |
| *13525114* | *Ernest Clarence Gunawan* |
| *13525117* | *Abdur Rauuf Fawaaz* |

### Catatan

| Catatan |
| --- |
| 1. *Terdapat kesalahan dalam identifikasi kelas, seharusnya dibagi ke beberapa jenis: Entity, Controller, Boundary*  |
| 2. *Pembuatan class diagram kembali disesuaikan dengan kelas yang baru, jenis panah yang digunakan diperhatikan sesuai arahan dalam asistensi akbar*  |


## Dokumentasi

<!-- ![](./assets/foto-asistensi.jpg) -->
<p align="center">
  <img src="./assets/diagram/foto-asistensi-m4.png" width="100%">
</p>

<p align="center">
  <i>Gambar 1. Dokumentasi kegiatan asistensi.</i>
</p>
